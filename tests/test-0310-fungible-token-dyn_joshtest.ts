...
[true, false].forEach((unlimited) => {
    describe('Suite: FungibleToken - Dynamic Supply ' + (unlimited ? "(Unlimited)" : "(Limited)"), function () {
        ...
        it("Burn - test alphanumeric value", function () {
            return fungibleToken.getBalance().then((balance) => {
                return fungibleToken.burn('abc123').then((receipt) => {
                    expect(receipt).is.not.exist;
                }).catch(error => {
                    expect(error.code).to.equal(errors.UNEXPECTED_RESULT]);
        ...
        it("Burn - test negative value", function () {
            return fungibleToken.getBalance().then((balance) => {
                return fungibleToken.burn(-10).then((receipt) => {
                    expect(receipt).is.not.exist;
                }).catch(error => {
